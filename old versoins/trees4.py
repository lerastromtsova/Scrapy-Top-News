...
class Tree:
    ...
    def find_last_topics(self):
        ...
        for node in self.roots:
            ...
            for document in node.documents:
                descr = set(document.tokens['title']) | set(document.tokens['lead'])
                common = descr.intersection(node.name)
                if common:
                    countries.add(document.country)
                if common == node.name:
                    has_all_words = True
            if len(countries) < 2 and has_all_words is False:
                to_remove.add(node)

        self.last_nodes = {n for n in self.roots if n not in to_remove}

        write_topics_to_xl("4-1.xlsx", self.last_nodes)
        write_tz(self)
    ...
    def unite_similar_topics(self):
        ...
        write_topics_with_freq("4-2.xlsx", self.roots, with_children=False)
    ...
    def unite_by_countries(self):
        ...
        write_topics_to_xl("4-3.xlsx", self.last_nodes, with_children=True)
