...
class Topic:
    ...
    def point_a(self):
        ...
        if count_countries(com_words) >= 1 and count_not_countries(com_words) >= 2:
            # count_countries(com_words) >= 2 and count_not_countries(com_words) >= 1:
    ...
    def point_c(self):
        ...
        for new in self.news:
            cw_in_tokens = new.descr_with_countries.intersection(self.name)
            ...
            try:
                ...
                if maxim != 0:
                    ...
                    for ms in most_similar:
                        if most_similar[ms] == max(self.frequencies[ms].values()):
                            mss = {row1, ms}
                            ...
                            name = {w for w in name if w[0].islower() or w in COUNTRIES}
...
if __name__ == '__main__':
    ...
    # c.find_trends()
    c.find_topics()
    write_topics(f"1-{date.today().day}-{date.today().month}(1).xlsx", c.topics)
    # c.delete_small()
    ...
    write_topics(f"2-{date.today().day}-{date.today().month}(1).xlsx", c.topics)

    # c.find_unique()
    # write_topics(f"6-{date.today().day}-{date.today().month}.xlsx", c.trends)

    c.final_delete()
    write_topics(f"3-{date.today().day}-{date.today().month}(1).xlsx", c.topics)
