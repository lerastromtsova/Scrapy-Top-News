...
from xl_stats import write_rows_content, write_rows_title, write_topics, write_news
...
class Corpus:
    ...
    def __init__(self, db, table):
        ...
        for row in self.data:
            if count_countries(row.named_entities['content']) == 2 and count_not_countries(row.named_entities['content']) == 0:
                self.two_countries.add(row)
            ...
            if maxim != 0:
                ...
                for ms in most_similar:
                    if self.frequencies[ms][row1] == max(self.frequencies[ms].values()):
                        ...
                        if count_countries(name) >= 1 and count_not_countries(name) >= 1:
        ...
        for row1 in self.data:
            ...
            if maxim != 0:
                ...
                for ms in most_similar:
                        ...
                        if count_countries(name) >= 1 and count_not_countries(name) >= 1 and mss not in self.similarities:
    ...
    def find_identic_topics(self):
        ...
        write_topics("темы-3.xlsx", self.topics)
    ...
    def find_unique(self):
        ...
        write_topics("темы-4.xlsx", self.trends)
...
if __name__ == '__main__':
    ...
    c = Corpus(db, table)
    write_news('two_countries.xlsx', c.two_countries)
