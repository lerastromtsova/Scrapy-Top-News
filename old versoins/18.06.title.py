...
class Corpus:
    ...
    def find_topics(self):
        ...
        for row1 in self.data:
            ...
            # frequencies[row1] = {row2: len(row1.description.intersection(row2.description))
            #                      for row2 in other_rows}

            frequencies[row1] = {row2: len({word.lower() for word in row1.tokens['title']}.intersection({w.lower()
                                for w in row2.tokens['title']})) for row2 in other_rows}
            ...
            if maxim != 0:
                ...
                for ms in most_similar:
                    if frequencies[ms][row1] == max(frequencies[ms].values()):
                        mss = [row1, ms]
                        name = {word.lower() for word in row1.tokens['title']}.intersection({w.lower()
                                for w in ms.tokens['title']})
                        if len(mss) > 1 and len(name)>1:
                            similarities.append(mss)

        write_rows_title("sim_topics.xlsx", similarities)
