...
""" Проверка с частотой между документами, без замены стран """
...
for n in nodes:
            frequent_words_dict = n.find_most_frequent()
...
while nodes:
    ...
    freq_words_1 = set(n.find_most_frequent().keys())
    similar = [n]
    while other_nodes:
        on = other_nodes.pop()

        freq_words_2 = set(on.find_most_frequent().keys())
        percent1 = 0
        percent2 = 0
